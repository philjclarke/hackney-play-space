...
if ( ! class_exists( 'UM_Gallery_Pro_AJAX' ) ) :
	...
	class UM_Gallery_Pro_AJAX {
		...
		public function um_gallery_add_videos() {
			if ( ! check_ajax_referer( 'um-gallery-nonce', 'security', false ) ) {
				do_action( 'um_gallery_video_update_fail' );
				$results['error']   = true;
				$results['message'] = __( 'Oops, something went wrong. Please try again.', 'um-gallery-pro' );
		...
		function um_gallery_album_update() {
			...
			$results           = array();
			if ( ! check_ajax_referer( 'um-gallery-nonce', 'security', false ) ) {
				do_action( 'um_gallery_album_update_fail' );
				$results['error']   = true;
				$results['message'] = __( 'Oops, something went wrong. Please try again.', 'um-gallery-pro' );
				wp_send_json( $results );
			}
			...
			$extra_fields      = ! empty( $_POST['extra_fields'] ) ? wp_kses_post( $_POST['extra_fields'] ) : '';
			...
			do_action( 'um_gallery_album_updated', $results, $extra_fields );
			wp_send_json( $results );
		}
		...
		function um_gallery_photo_upload() {
			if ( ! check_ajax_referer( 'um-gallery-nonce', 'security', false ) ) {
				do_action( 'um_gallery_photo_upload_fail' );
				$results['error']   = true;
				$results['message'] = __( 'Oops, something went wrong. Please try again.', 'um-gallery-pro' );
				wp_send_json( $results );
			}
			...
			if ( ! empty( $_FILES ) ) :
				...
				if ( is_user_logged_in() ) {
					...
					$file       = isset( $file['file'] ) ? $file['file'] : $file;
					$album_id   = (int) $_POST['album_id'];
		...
		function um_gallery_photo_update() {
			if ( ! check_ajax_referer( 'um-gallery-nonce', 'security', false ) ) {
				do_action( 'um_gallery_photo_update_fail' );
				$results['error']   = true;
				$results['message'] = __( 'Oops, something went wrong. Please try again.', 'um-gallery-pro' );
				wp_send_json( $results );
			}
		...
		function um_gallery_ajax_delete_album() {
			if ( ! check_ajax_referer( 'um-gallery-nonce', 'security', false ) ) {
				do_action( 'um_gallery_album_delete_fail' );
				$results['error']   = true;
				$results['message'] = __( 'Oops, something went wrong. Please try again.', 'um-gallery-pro' );
				wp_send_json( $results );
			}
		...
		public function delete_item() {
			if ( ! check_ajax_referer( 'um-gallery-nonce', 'security', false ) ) {
				do_action( 'um_gallery_object_delete_fail' );
				$results['error']   = true;
				$results['message'] = __( 'Oops, something went wrong. Please try again.', 'um-gallery-pro' );
				wp_send_json( $results );
			}
