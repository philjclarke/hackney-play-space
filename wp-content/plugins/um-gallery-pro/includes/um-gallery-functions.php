...
function um_gallery_fix_image_orientation( $file = array() ) {
	...
	if ( ! empty( $file['file']['tmp_name'] ) && ! file_exists( $file['file']['tmp_name'] ) ) {
		return $file;
	}
	error_log( __LINE__ . ' ' . print_r( $file, true ) );
	// Attempt to read EXIF data from the image
	$exif_data = wp_read_image_metadata( $file['file']['tmp_name'] );
	if ( ! $exif_data ) {
		return $file;
	}
