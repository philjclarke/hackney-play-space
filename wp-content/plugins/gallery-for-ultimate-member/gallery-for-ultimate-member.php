...
Text Domain: gallery-for-ultimate-member
*/

// Exit if accessed directly
if ( ! defined( 'ABSPATH' ) ) exit;

define( 'UM_GALLERY_LITE_URL', plugin_dir_url( __FILE__ ) );
define( 'UM_GALLERY_LITE_PATH', plugin_dir_path( __FILE__ ) );
define( 'UM_GALLERY_LITE_PLUGIN', plugin_basename( __FILE__ ) );
define( 'UM_GALLERY_LITE_VERSION', '1.0.5.8.1' );
	...
	class UM_Gallery {
		...
		**/
		...
		public function plugin_classes() {
			require_once( UM_GALLERY_LITE_PATH . 'vendor/cmb2/cmb2/init.php' );
			require_once( UM_GALLERY_LITE_PATH . 'includes/um-gallery-admin-list.php' );
			require_once( UM_GALLERY_LITE_PATH . 'includes/um-gallery-admin.php' );
			require_once( UM_GALLERY_LITE_PATH . 'includes/class-um-gallery-template.php' );
			require_once( UM_GALLERY_LITE_PATH . 'includes/um-gallery-ajax.php' );
			require_once( UM_GALLERY_LITE_PATH . 'includes/class-um-gallery-field.php' );
			require_once( UM_GALLERY_LITE_PATH . 'includes/um-gallery-shortcodes.php' );
			require_once( UM_GALLERY_LITE_PATH . 'includes/um-gallery-functions.php' );
			require_once( UM_GALLERY_LITE_PATH . 'includes/um-gallery-comments.php' );
			require_once( UM_GALLERY_LITE_PATH . 'includes/class-um-gallery-privacy.php' );

			// Include widgets
			require_once( UM_GALLERY_LITE_PATH . 'includes/widgets/class-widget-recent-photos.php' );

			$this->admin 		= new UM_GalleryPro_Admin();
			$this->comments 	= new UM_Gallery_Lite_Comments();
			$this->ajax 		= new UM_Gallery_Lite_AJAX();
			$this->field 		= new UM_Gallery_Lite_Field();
			$this->template		= new UM_Gallery_Lite_Template();
			$this->shortcode	= new UM_Gallery_Lite_Shortcodes();
		}
		...
		public function load_plugin_textdomain() {
			...
			$loaded = load_plugin_textdomain( 'gallery-for-ultimate-member', false, '/languages/' );

			if ( ! $loaded ) {
				$loaded = load_muplugin_textdomain( 'gallery-for-ultimate-member', '/languages/' );
			}

			if ( ! $loaded ) {
				$locale = apply_filters( 'plugin_locale', get_locale(), 'gallery-for-ultimate-member' );
				$mofile = dirname( __FILE__ ) . '/languages/gallery-for-ultimate-member-' . $locale . '.mo';
				load_textdomain( 'gallery-for-ultimate-member', $mofile );
			}
		}
		...
		public function add_scripts() {
			...
			wp_enqueue_style(
				'um-gallery-style',
				plugins_url( '/assets/css/um-gallery' . $suffix . '.css' , __FILE__ )
			);
			 ...
			 * @return null
			 */
			wp_register_script( 'um_gallery', um_gallery()->plugin_url . 'assets/js/um-gallery' . $suffix . '.js', array( 'jquery','jquery-ui-core', 'jquery-ui-widget', 'jquery-ui-autocomplete', 'jquery-ui-position' ), UM_GALLERY_LITE_VERSION, true );
			...
			// Comment text.
			$localization['comments']['textareaPlaceholderText'] = esc_html__( 'Add a comment', 'gallery-for-ultimate-member' );
			$localization['comments']['newestText']              = esc_html__( 'Newest', 'gallery-for-ultimate-member' );
			$localization['comments']['oldestText']              = esc_html__( 'Oldest', 'gallery-for-ultimate-member' );
			$localization['comments']['popularText']             = esc_html__( 'Popular', 'gallery-for-ultimate-member' );
			$localization['comments']['attachmentsText']         = esc_html__( 'Attachments', 'gallery-for-ultimate-member' );
			$localization['comments']['sendText']                = esc_html__( 'Send', 'gallery-for-ultimate-member' );
			$localization['comments']['replyText']               = esc_html__( 'Reply', 'gallery-for-ultimate-member' );
			$localization['comments']['editText']                = esc_html__( 'Edit', 'gallery-for-ultimate-member' );
			$localization['comments']['editedText']              = esc_html__( 'Edited', 'gallery-for-ultimate-member' );
			$localization['comments']['youText']                 = esc_html__( 'You', 'gallery-for-ultimate-member' );
			$localization['comments']['saveText']                = esc_html__( 'Save', 'gallery-for-ultimate-member' );
			$localization['comments']['deleteText']              = esc_html__( 'Delete', 'gallery-for-ultimate-member' );
			$localization['comments']['viewAllRepliesText']      = esc_html__( 'View all __replyCount__ replies', 'gallery-for-ultimate-member' );
			$localization['comments']['hideRepliesText']         = esc_html__( 'Hide replies', 'gallery-for-ultimate-member' );
			$localization['comments']['noCommentsText']          = esc_html__( 'No comments', 'gallery-for-ultimate-member' );
			$localization['comments']['noAttachmentsText']       = esc_html__( 'No attachments', 'gallery-for-ultimate-member' );
			$localization['comments']['attachmentDropText']      = esc_html__( 'Drop files here', 'gallery-for-ultimate-member' );
		...
		public function get_user_image_src( $photo = array(), $size = 'thumbnail', $force = false, $type = 'photo' ) {
			...
			// Get the name.
			$name = ! empty( $photo->file_name ) ? $photo->file_name : '';
...
if ( ! function_exists( '_um_gallery_pro_activate' ) ) {
	function _um_gallery_pro_activate( $network_wide ) {
		...
		update_option( 'um_gallery_version', '1.0.5.7' );
	}
}
...
if ( ! function_exists( 'um_gallery_pro_new_blog_setup' ) ) {
	function um_gallery_pro_new_blog_setup( $blog_id, $user_id, $domain, $path, $site_id, $meta ) {
		...
		if ( is_plugin_active_for_network( 'gallery-for-ultimate-member/um-gallery.php' ) ) {
			$old_blog = $wpdb->blogid;
			switch_to_blog( $blog_id );
			um_gallery_pro_db_setup();
			switch_to_blog( $old_blog );
...
if ( ! function_exists( 'um_gallery_pro_db_setup' ) ) {
	function um_gallery_pro_db_setup() {
		...
		$default_options = array(
			'profile_tab_gallery'         => 1,
			'profile_tab_gallery_privacy' => 0,
		);
