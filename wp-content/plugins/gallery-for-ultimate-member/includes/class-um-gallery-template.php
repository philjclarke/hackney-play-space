...
if ( ! class_exists( 'UM_Gallery_Pro_Template' ) ) :
	...
	class UM_Gallery_Lite_Template {
		...
		public function hooks() {
			...
			add_filter( 'um_user_profile_tabs', array( $this, 'setup_gallery_tabs' ), 12, 1 );
		...
		public function setup_gallery_tabs( $tabs = array() ) {
			...
			if ( is_admin() || um_gallery_can_moderate() ) {
				$title  = um_gallery_pro_get_option( 'um_gallery_tab_name', __( 'Gallery', 'gallery-for-ultimate-member' ) );
		...
		public function get_profile_photos_view() {
			...
			$amount = um_gallery_pro_get_option( 'um_gallery_profile_count', 10 );
			$atts = array(
				'user_id'  => $user_id,
				'per_load' => $amount,
			);
		...
		public function get_profile_single_album_view( $album_id = 0 ) {
			...
			<a href="<?php echo um_gallery_profile_url(); ?>" class="um-gallery-btn"><i class="um-faicon-chevron-left"></i> <?php _e('Back to Albums', 'gallery-for-ultimate-member'); ?>
			</a>
			...
			<?php if( ! um_gallery()->template->quick_upload ): ?>
			<div class="um-gallery-album-head">
				<h3 class="um-gallery-album-title"><?php echo esc_html( $album->album_name, 'gallery-for-ultimate-member' ); ?></h3>
				<?php if ( ! empty( $album->album_description ) ): ?>
				<div class="um-gallery-album-description"><?php echo esc_html( $album->album_description ); ?></div>
				<?php endif; ?>
			</div>
