...
class User_Photos_Ajax {
	...
	function __construct() {
		...
		add_action('um_user_photos_after_photo_deleted', array( $this, 'update_album_images'), 10, 2 );
	...
	function create_um_user_photos_album() {
		...
		if ( false !== $limit_user_photos && isset( $_FILES['album_images'] ) ) {
			...
			if ( (int) $count_user_photos + (int) $count_files > $limit_user_photos ) {
				$response['messages'][] = sprintf( esc_html__( 'You cannot upload more photos, you have reached the limit of uploads. Delete old photos or upload few photos. Photo limit = %d', 'um-user-photos' ), $limit_user_photos );
				echo json_encode( $response );
				die;
			}
	...
	/**
	 * Update album images meta when image is deleted
	 * @params int $image_id
	 * @params int $album_id
	 */

	function update_album_images( $image_id, $album_id ){

		$arr1 = array( $image_id );
		$arr2 = get_post_meta( $album_id, '_photos', true );

		if( $arr2 && is_array( $arr2 ) ){
			$data = array_diff( $arr2,$arr1 );
			update_post_meta( $album_id,'_photos',$data);
		}
