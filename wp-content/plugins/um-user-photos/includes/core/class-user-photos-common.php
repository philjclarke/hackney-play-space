...
class User_Photos_Common {
	...
	function __construct() {
		...
		add_action('pre_get_posts', array( $this, 'hide_list_view_user_photos' ), 10, 1 );
	...
	/*
	 * Hide user photos on media library list view
	 * @param &query
	*/
	
	function hide_list_view_user_photos( $query ){
	 ...
	 */
	function remove_language_filter( $query ) {
		$qv = &$query->query_vars;
		if ( !empty( $qv['tax_query'] ) && is_array( $qv['tax_query'] ) ) {
			foreach ( $qv['tax_query'] as $key => $q ) {
				if ( isset( $q['taxonomy'], $q['terms'] ) && $q['taxonomy'] === 'language' ) {
	...
	public function photos_count( $user_id ) {
		...
		foreach ( $albums as $album ) {
			...
			$count_album_photos = count( $album_photos_array );
			$count              = $count + $count_album_photos;
		}
	...
	public function photos_limit( $user_id ) {
		...
		$limit_user_photos = ! empty( $role_data['limit_user_photos'] ) ? $role_data['limit_user_photos'] : false;

		return $limit_user_photos;
	}
}
