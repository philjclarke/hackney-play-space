...
if ( 1 === (int) $defaults['um_header_profile_position'] ) {
...
if ( ! function_exists( 'um_theme_header_notification_modal' ) ) {
    function um_theme_header_notification_modal() {
        ...
        if (  1 !== (int) $defaults['um_show_header_notification'] ) {
            return;
        }
        ...
        $new_notifications_array = count( $new_notifications_array );
        $unread_count            = ( absint( $new_notifications_array ) > 9 ) ? __( '9+', 'um-theme' ) : $new_notifications_array;
        ?>

        <div class="header-notification-box">
            <div class="dropdown msg-drop" aria-label="<?php esc_attr_e( 'Notifications', 'um-theme' );?>" data-balloon-pos="down">
                ...
                <span class="um-notification-live-count">
                    <?php echo ( (int) $unread_count !== 0 ) ? absint( $unread_count ) : ''; ?>
                </span>
                <ul class="um-notification-live-feed dropdown-menu msg-drop-menu" aria-labelledby="dropdownMenuButton">
                    <?php
...
function um_theme_um_groups_directory( $args ){
    ...
    if ( um_groups('total_groups') > 0 ) {
        ...
        foreach( um_groups('groups') as $group ):
                ...
                echo '<div class="um-group-item">';
                    ...
                    if( true == $args['show_actions'] ) {
                        ...
                        echo '<ul>';
                            ...
                            echo '<span class="group-meta-info">';
                                echo '<span>';
                                echo __('active, ','um-theme').human_time_diff( UM()->Groups()->api()->get_group_last_activity( $group->ID, true ), current_time( 'timestamp' ) ).__(' ago','um-theme');
                                echo '</span>';
                            echo '</span>';
                            echo '</li>';
                ...
                ?>
                    <?php if ( 2 === (int) $defaults['um_theme_ext_um_group_list_layout'] ) : ?>
                                    ...
                                    <div class="actions">
                                        <ul>
                                            ...
                                            <li class="last-active">
                                                ...
                                                <span><?php echo human_time_diff( UM()->Groups()->api()->get_group_last_activity( $group->ID, true ), current_time( 'timestamp' ) ).__(' ago','um-theme');?></span>
                                                </span>
                                            </li>
                                        </ul>
                                    </div>
                    ...
                    <?php if ( 3 === (int) $defaults['um_theme_ext_um_group_list_layout'] ) : ?>
...
if ( ! function_exists( 'um_theme_um_groups_directory_search_form' ) ) {
    function um_theme_um_groups_directory_search_form( $args ){
        ...
        <form class="um-groups-search-form" style="display:none;">
        ...
        <div class="boot-row">
            ...
            <div class="group-search-filter boot-col-md-7">
                ...
                echo '<a href="'. esc_url( $clear_link ) .'" class="primary-button">'.__('Clear', 'um-theme' ).'</a>';
                ?>
            </div>
        </div>
        <input type="hidden" name="profiletab" value="groups_list">
        </form>
         <div class="um-clear"></div>
        </div>
        <?php
...
if ( ! function_exists( 'um_theme_friend_box_profile' ) ) {
    function um_theme_friend_box_profile() {
        ...
        // If plugin - 'Friends Extension' not exist then return.
        if ( ! class_exists( 'UM_Friends_API' ) ) {
            return;
        }

        if ( 1 === (int) $defaults['um_show_profile_friend_requests'] ) {
...
if ( function_exists( 'um_messaging_plugins_loaded' ) ) {
    ...
    // If plugin - 'Messaging Extension' not exist then return.
    if ( ! class_exists( 'UM_Messaging_API' ) ) {
        return;
    }

    if ( 2 === (int) $defaults['um_theme_ext_pm_message_hide_chat_history'] ) {
...
if ( ! function_exists( 'um_get_conversation' ) ) {
    ...
    $selector_data = (int) $defaults['um_displayname_header_messenger'];
