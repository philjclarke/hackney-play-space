	...
	public function widget( $args, $instance ) {
		...
		<?php
			...
			if ( $title ) {
				...
				if ( $um_member_order == 2 ) {
					$query_args['orderby'] = 'user_name';
				}

				if ( $um_member_order == 3 ) {
					$query_args['orderby'] = 'display_name';
				}

				if ( $um_member_order == 4 ) {
				...
				if ( empty( $users ) ) {
					esc_html_e( 'User Not Found', 'um-theme' );
				}
			...
			<label for="<?php echo $this->get_field_id( 'role' ); ?>"><?php _e( 'Role:', 'um-theme' ); ?></label>
				<select class="widefat" id="<?php echo $this->get_field_id( 'role' ); ?>" name="<?php echo $this->get_field_name( 'role' ); ?>">
					<?php 
						...
						wp_dropdown_roles( $instance['role'] ); // Dropdown list of roles. 
					?>
				</select>
		</p>
