...
if ( ! function_exists( 'um_theme_customizer_head_styles' ) ) {
	function um_theme_customizer_head_styles() {
		...
		$css .= '
			...
			.header__wrapper{max-width:' . esc_attr( $header_width ) . ';}
			...
			.profile-menu-wrapper .dropdown-menu,
		...
		if ( $defaults['um_show_header_search'] === 1 || $defaults['um_show_header_search'] === 2 ) :
			$css .='
				...
				.header__wrapper .um-search-field ::placeholder,
				.header__wrapper .search-form ::placeholder{
		...
		if ( $defaults['um_theme_submenu_arrow'] === 2 ) :
			$css .='
				.header-profile-section .fa-angle-down,.dropdown-toggle::after{
					display:none;
				}
			';
		endif;
		...
		if ( $defaults['um_theme_user_bookmarks_list_excerpt_show'] === 2 ) :
			$css .= '.um-user-bookmarks-post__content-content p{ display:none;}';
		endif;
		...
		if ( function_exists( 'um_reviews_plugins_loaded' ) ) :
			...
			$css .= '
				...
				div.um .um-form div.um-reviews-post__content .um-reviews-title{color:' . sanitize_hex_color( $defaults['um_theme_ext_reviews_single_title_color'] ) . ' !important;}
